            ...
            if isinstance(batch_data[0][key], dict):
                ...
                for key2 in batch_data[0][key].keys():
                    if isinstance(batch_data[0][key][key2], dict):
                        ...
                        item_data[key][key2] = torch.Tensor(np.stack([sample[key][key2] for sample in batch_data])).to(device=self.device, dtype=torch.float32)
            else:
